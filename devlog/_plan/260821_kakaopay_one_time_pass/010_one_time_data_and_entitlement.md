...
| Path | Change |
| --- | --- |
| `dev_poc/poc-4-edge-function/supabase/migrations/20260821010000_add_kakaopay_one_time_pass.sql` | Add `kakaopay_one_time_orders` and `kakaopay_one_time_entitlements`, RLS, indexes, update trigger, and an amendment to `recompute_member_plan`. |
...
- `kakaopay_one_time_orders`: immutable `product_key`, server price/order name, `order_id`, user, hashed partner user id, `tid`, `aid`, provider status, terminal/repair states, timestamps, and refund fields. Unique provider IDs prevent an approval from being credited twice.
- `kakaopay_one_time_entitlements`: one row per successful order, `starts_at`, `ends_at`, `status` (`active`, `refunded`, `expired`), and the source order id. Its end time is calculated from the server catalog at approval time.
- `recompute_member_plan` treats an active unexpired one-time entitlement exactly like an unexpired subscription when deriving `members.plan`; it never writes entitlement time from browser data.
- The ready path rejects an existing active entitlement or recurring Pro access. It does not extend access silently. Renewal is a fresh, later purchase after expiry.
...
4. Mark the order full-refunded through the cancel harness, query the entitlement status, and invoke `member-get`; assert `status = 'refunded'` and `plan === "free"` unless the fixture also contains a valid recurring entitlement.
5. Run the targeted Deno test file named in phase 020. Keep fixture values generated locally and never use a production user, CID, or secret.
