...
| `dev_poc/poc-4-edge-function/supabase/functions/kakaopay-one-time-approve/index.ts` | Return the server-computed entitlement end timestamp only after the atomic approval RPC succeeds, so the success page never calculates paid access in the browser. |
...
- The success page receives `endsAt` only from a successful one-time approval response; a missing value shows a reconciliation message, never an invented access date.
...
3. With `VITE_KAKAOPAY_ONETIME_ENABLED` unset, use Aside Browser on the signed-in review route, press the one-time payment action, capture the post-click snapshot and network log, and assert there is no request to `kakaopay-one-time-ready` or a Kakao URL.
